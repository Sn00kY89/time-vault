...
export default function App() {
  ...
  useEffect(() => {
    ...
    const loadUserPrefs = async () => {
      try {
        const snap = await getDoc(doc(db, 'artifacts', APP_ID, 'users', user.uid, 'settings', 'theme'));
        if (snap.exists()) {
  ...
  useEffect(() => {
    ...
    const q = collection(db, 'artifacts', APP_ID, 'users', user.uid, 'work_logs');
    const unsubscribe = onSnapshot(q, (snapshot) => {
        const data = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
        // Filtro robusto per evitare crash con dati corrotti
        setLogs(data.filter(d => d.date).sort((a, b) => new Date(b.date) - new Date(a.date)));
      },
    ...
    const checkWeekendAutomation = async () => {
      ...
      if (dayOfWeek >= 1) { 
        ...
        for (const dateStr of datesToCheck) {
          ...
          if (!exists) {
            try {
              await addDoc(collection(db, 'artifacts', APP_ID, 'users', user.uid, 'work_logs'), {
  ...
  const handleLogSubmit = async (e) => {
    ...
    try {
      ...
      setSelectedLeaders([]); setView('calendar');
    } catch (e) {}
  };
  ...
  const confirmDownload = async () => {
    ...
    try {
      const canvas = await window.html2canvas(reportElement, { scale: 2, useCORS: true, backgroundColor: '#ffffff' });
  ...
  const handleDeleteAccount = async () => {
    setIsDeleting(true);
    try { await deleteUser(user); window.location.reload(); } catch (e) { alert("Riautenticati per eliminare l'account."); setIsDeleting(false); }
  };
  ...
  const monthlyStats = useMemo(() => {
    ...
    currentMonthLogs.forEach(log => {
        if (!['ferie', 'malattia', 'riposo'].includes(log.type)) uniqueDays.add(log.date);
      ...
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex items-center justify-center p-6 relative overflow-hidden">
        ...
        <div className="bg-white dark:bg-slate-900 p-10 rounded-[2.5rem] w-full max-w-md shadow-2xl border border-slate-200 dark:border-slate-800 text-center relative animate-in zoom-in duration-300">
          <div className="mb-12 mt-6 text-center">
             ...
             <p className="text-[10px] text-slate-400 font-black uppercase tracking-[0.3em] italic leading-none">Personal Workplace</p>
          </div>
          ...
          <button onClick={() => setShowGuideModal(true)} className="mt-12 text-[10px] font-black text-slate-400 uppercase tracking-widest hover:text-blue-500 flex items-center justify-center gap-2 mx-auto italic"><HelpCircle size={14} /> Guida WebApp</button>
        </div>
             ...
             <div className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] w-full max-w-sm shadow-2xl border relative animate-in zoom-in-95 duration-200 border-slate-200 dark:border-slate-800">
                ...
                {isLocked ? (
                   <div className="text-center">
                      ...
                      <p className="text-[10px] text-slate-400 font-bold uppercase mb-6 italic tracking-widest leading-none">Usa la chiave di recupero</p>
                      {recoveryStep === 1 ? (
                        <div className="mt-6 space-y-4">
                           <input type="text" placeholder="XXXX-XXXX-XXXX-XXXX" className="w-full p-4.5 bg-red-50 dark:bg-red-900/10 text-red-900 font-mono text-center font-bold outline-none border border-red-100 rounded-2xl" value={unlockCodeInput} onChange={e => setUnlockCodeInput(e.target.value.toUpperCase())} />
                           <button onClick={() => setRecoveryStep(2)} className="w-full bg-red-600 text-white p-5 rounded-2xl font-black uppercase tracking-widest shadow-lg active:scale-95 transition-all">Verifica</button>
                        </div>
                      ) : (
                        <div className="mt-6 space-y-4 animate-in slide-in-from-bottom duration-300">
                           ...
                           <button onClick={() => { setIsLocked(false); setFailedAttempts(0); setShowAuthModal(false); alert("Vault Sbloccato!"); }} className="w-full bg-blue-600 text-white p-5 rounded-2xl font-black uppercase shadow-lg active:scale-95 transition-all">Ripristina</button>
                        </div>
                      )}
                   </div>
                ) : (
                   ...
                   <div className="text-center mb-8">
                     ...
                     <h2 className="text-2xl font-black italic uppercase text-slate-900 dark:text-white tracking-tight leading-none">{authMode === 'login' ? 'Bentornato' : 'Nuovo Utente'}</h2>
                   </div>
                   <form onSubmit={handleAuth} className="space-y-4">
                     <input type="text" placeholder="nome.cognome" required className="w-full p-4.5 bg-slate-100 dark:bg-slate-800 dark:text-white rounded-2xl font-bold outline-none border border-slate-200 focus:ring-2 focus:ring-blue-500/20" value={authData.username} onChange={e => setAuthData({...authData, username: e.target.value})} />
                     <input type="password" placeholder="••••••••" required className="w-full p-4.5 bg-slate-100 dark:bg-slate-800 dark:text-white rounded-2xl font-bold outline-none border border-slate-200 focus:ring-2 focus:ring-blue-500/20" value={authData.password} onChange={e => setAuthData({...authData, password: e.target.value})} />
                     {authError && <div className="text-red-600 text-[11px] font-black italic text-center uppercase">{authError}</div>}
                     <button type="submit" disabled={isSubmitting} className={`w-full text-white p-5 rounded-2xl font-black uppercase tracking-widest bg-${accentColor}-600 shadow-xl active:scale-95 transition-all`}>
                       {isSubmitting ? <Loader2 className="animate-spin mx-auto" /> : 'Sblocca Vault'}
                     </button>
                   </form>
        ...
        {showRecoveryModal && (
           <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-[110] flex items-center justify-center p-4 animate-in fade-in duration-300">
              <div className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] w-full max-w-sm border-2 border-red-500 text-center shadow-2xl">
                 ...
                 <p className="text-[10px] text-slate-500 font-black uppercase mb-6 leading-relaxed italic tracking-widest">Salva questo codice ora. È l'unico modo per recuperare l'account.</p>
                 <div className="bg-slate-100 dark:bg-slate-950 p-5 rounded-2xl mb-6 font-mono text-lg font-black tracking-widest break-all dark:text-white shadow-inner">{generatedRecoveryCode}</div>
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 transition-colors duration-300">
          ...
          <div className="space-y-10 animate-in fade-in zoom-in duration-300">
            <div className="grid grid-cols-2 gap-6 text-left">
              <div className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] shadow-sm border border-slate-200 dark:border-slate-800 hover:shadow-xl transition-all overflow-hidden relative">
                <div className={`absolute top-0 right-0 p-6 opacity-10 text-${accentColor}-600`}><CalendarIcon size={80}/></div>
              ...
              <div className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] shadow-sm border border-slate-200 dark:border-slate-800 hover:shadow-xl transition-all overflow-hidden relative">
                <div className="absolute top-0 right-0 p-6 opacity-10 text-orange-600"><Zap size={80}/></div>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-[3rem] shadow-2xl border border-slate-200 dark:border-slate-800 p-8 md:p-12 overflow-hidden relative">
              <div className="flex flex-col sm:flex-row items-center justify-between mb-12 gap-6 relative z-10 text-left">
                ...
                <div className="flex bg-slate-100 dark:bg-slate-800 p-2 rounded-2xl gap-1 border border-slate-200 dark:border-slate-700 shadow-inner">
                  <button onClick={() => setCurrentMonth(new Date(currentMonth.getFullYear(), currentMonth.getMonth() - 1, 1))} className="p-3 hover:bg-white rounded-xl text-slate-500 shadow-sm active:scale-90"><ChevronLeft size={22}/></button>
                  <button onClick={() => setCurrentMonth(new Date())} className="px-4 text-[10px] font-black uppercase tracking-widest text-slate-400 italic">Oggi</button>
                  <button onClick={() => setCurrentMonth(new Date(currentMonth.getFullYear(), currentMonth.getMonth() + 1, 1))} className="p-3 hover:bg-white rounded-xl text-slate-500 shadow-sm active:scale-90"><ChevronRight size={22}/></button>
                </div>
              </div>
              ...
              <div className="grid grid-cols-7 gap-4 md:gap-6 relative z-10">
                ...
                {Array.from({ length: days }).map((_, i) => {
                  ...
                  return (
                    <button key={day} onClick={() => { setSelectedDate(new Date(currentMonth.getFullYear(), currentMonth.getMonth(), day)); setView('day'); }} className={`aspect-square rounded-[1.5rem] md:rounded-[2rem] flex flex-col items-center justify-center relative transition-all group ${active ? `bg-${accentColor}-600 text-white shadow-xl shadow-${accentColor}-500/20 scale-100` : isToday ? 'bg-blue-50 dark:bg-blue-900/20 ring-2 ring-blue-500/30 text-blue-600' : 'bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 hover:scale-105 active:scale-95 shadow-inner'}`}>
                      <span className={`text-xl font-black tracking-tighter leading-none`}>{day}</span>
        ...
        {view === 'day' && (
          <div className="space-y-8 animate-in slide-in-from-right duration-300 text-left">
            ...
            <div className="bg-white dark:bg-slate-900 p-8 md:p-12 rounded-[3rem] shadow-2xl border border-slate-200 dark:border-slate-800 relative overflow-hidden">
              ...
              <h3 className="text-[11px] font-black mb-10 uppercase text-slate-400 tracking-[0.3em] italic leading-none border-l-4 border-blue-500 pl-4 text-left">Registrazione Log</h3>
              
              <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-10 text-left">
                 {!isWeekend && (
                   <>
                     <button type="button" onClick={() => setFormData(p => ({ ...p, standardHours: STANDARD_HOURS_VALUE, type: 'work' }))} className={`p-5 rounded-3xl font-black uppercase text-[9px] flex flex-col items-center gap-3 transition-all ${formData.type === 'work' && formData.standardHours === STANDARD_HOURS_VALUE ? `bg-${accentColor}-600 text-white shadow-xl shadow-${accentColor}-500/20 scale-105` : 'bg-slate-50 dark:bg-slate-800 text-slate-400'}`}><Briefcase size={24} />Standard</button>
                     <button type="button" onClick={() => setFormData({ standardHours: 0, overtimeHours: '', notes: 'Ferie', type: 'ferie' })} className={`p-5 rounded-3xl font-black uppercase text-[9px] flex flex-col items-center gap-3 transition-all ${formData.type === 'ferie' ? 'bg-emerald-500 text-white shadow-xl' : 'bg-slate-50 dark:bg-slate-800 text-slate-400'}`}><Palmtree size={24} />Ferie</button>
                   </>
                 )}
                 <button type="button" onClick={() => setFormData({ standardHours: 0, overtimeHours: '', notes: 'Malattia', type: 'malattia' })} className={`p-5 rounded-3xl font-black uppercase text-[9px] flex flex-col items-center gap-3 transition-all ${formData.type === 'malattia' ? 'bg-pink-500 text-white shadow-xl' : 'bg-slate-50 dark:bg-slate-800 text-slate-400'}`}><Thermometer size={24} />Malattia</button>
                 <button type="button" onClick={() => setFormData({ standardHours: 0, overtimeHours: '', notes: 'Riposo Compensativo', type: 'riposo_compensativo' })} className={`p-5 rounded-3xl font-black uppercase text-[9px] flex flex-col items-center gap-3 transition-all ${formData.type === 'riposo_compensativo' ? 'bg-indigo-500 text-white shadow-xl' : 'bg-slate-50 dark:bg-slate-800 text-slate-400'}`}><Coffee size={24} />Riposo Comp.</button>
                 <button type="button" onClick={() => setShowOvertimeInput(!showOvertimeInput)} className={`p-5 rounded-3xl font-black uppercase text-[9px] flex flex-col items-center gap-3 transition-all ${showOvertimeInput ? 'bg-orange-500 text-white shadow-xl' : 'bg-slate-50 dark:bg-slate-800 text-slate-400'}`}><Zap size={24} />Extra</button>
              </div>

              <form onSubmit={handleLogSubmit} className="space-y-8 animate-in fade-in duration-300 relative z-10 text-left">
                {showOvertimeInput && (
                   <div className="animate-in slide-in-from-top-2 duration-300">
                     <label className="block text-[10px] font-black text-orange-500 uppercase tracking-widest mb-3 ml-1 italic leading-none">Ore Straordinario (es. 2.5)</label>
                     <input type="number" step="0.5" className="w-full p-5 bg-orange-50 dark:bg-orange-900/10 text-orange-600 rounded-3xl font-black text-2xl outline-none border border-orange-100 shadow-inner" placeholder="0.0" value={formData.overtimeHours} onChange={e => setFormData({...formData, overtimeHours: e.target.value})} />
                   </div>
                )}
                <div className="border-t border-slate-100 dark:border-slate-800 pt-8 text-center">
                  <button type="button" onClick={() => setShowNotesInput(!showNotesInput)} className="text-slate-300 dark:text-slate-600 hover:text-slate-600 p-2 flex items-center gap-2 mx-auto uppercase text-[10px] font-black tracking-widest italic leading-none transition-colors">{showNotesInput ? <><ChevronUp size={16}/> Nascondi Note</> : <><ChevronDown size={16}/> Note e Responsabili</>}</button>
                  {showNotesInput && (
                          ...
                          {isLeaderDropdownOpen && (
                              <div className="absolute top-full left-0 right-0 mt-3 bg-white dark:bg-slate-800 border dark:border-slate-700 rounded-[2rem] shadow-2xl z-[60] p-3 max-h-60 overflow-y-auto animate-in zoom-in-95 text-left">
                                  {availableLeaders.map(l => (
                                      <div key={l} onClick={() => {
                                          const next = selectedLeaders.includes(l) ? selectedLeaders.filter(x => x !== l) : [...selectedLeaders, l];
                                          setSelectedLeaders(next);
                                      }} className="flex items-center gap-4 p-4 rounded-2xl hover:bg-slate-50 dark:hover:bg-slate-700 cursor-pointer transition-all">
                ...
                {formError && <p className="text-red-600 text-[10px] font-black italic mt-4 uppercase tracking-widest text-center animate-bounce leading-none">{formError}</p>}
                 ...
                 <div key={log.id} className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between shadow-lg relative group overflow-hidden hover:shadow-xl transition-all animate-in slide-in-from-right duration-500">
                    ...
                    <div className="space-y-4">
                       <div className="flex flex-wrap items-center gap-3">
                          <span className={`text-[10px] font-black uppercase bg-slate-100 dark:bg-slate-800 px-3 py-1.5 rounded-xl italic tracking-[0.2em] text-slate-500 leading-none`}>{(log.type || 'N/D').replace('_', ' ')}</span>
                    ...
                    <div className="flex justify-end mt-6 sm:mt-0">
                       <button onClick={() => setLogToDelete(log.id)} className="p-4 text-slate-200 hover:text-red-500 rounded-2xl transition-all hover:bg-red-50 dark:hover:bg-red-900/10 group-hover:text-slate-400 active:scale-90"><Trash2 size={20} /></button>
                    </div>
                 </div>
               ))}
            </div>
                         ...
                         <div key={log.id} className={`bg-slate-50/50 dark:bg-slate-800/20 rounded-[2rem] border-2 transition-all cursor-pointer hover:bg-white dark:hover:bg-slate-800 shadow-sm ${expandedLogId === log.id ? `ring-4 ring-${accentColor}-500/10 border-blue-500/30 scale-[1.02] bg-white dark:bg-slate-800` : 'border-transparent'}`} onClick={() => setExpandedLogId(expandedLogId === log.id ? null : log.id)}>
                            <div className="p-6 md:p-8 flex items-center justify-between">
                                <div className="flex items-center gap-6">
                                   ...
                                   <div>
                                      <p className={`text-[10px] font-black uppercase text-slate-400 mb-1 tracking-[0.3em] italic leading-none`}>{(log.type || 'N/D').replace('_', ' ')}</p>
                            ...
                            {expandedLogId === log.id && (
                                <div className="px-8 pb-10 pt-4 border-t border-slate-100 dark:border-slate-700/50 animate-in slide-in-from-top-4 duration-500">
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                                       ...
                                       <div className="space-y-3">
                                          <p className={`text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] italic flex items-center gap-2 leading-none`}><FileText size={16}/> Note Registrate</p>
                                          <p className="text-sm font-medium text-slate-500 dark:text-slate-400 pl-6 italic leading-relaxed">{log.notes || "Nessuna nota."}</p>
                                       </div>
                                    </div>
                                </div>
                            )}
            ...
            <div className="bg-white dark:bg-slate-900 p-8 md:p-14 rounded-[4rem] shadow-2xl border border-slate-200 dark:border-slate-800 space-y-14 relative overflow-hidden">
               <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-12 gap-8 relative z-10 text-left">
                  <div className="space-y-2">
                    ...
                    <p className="text-[11px] text-slate-400 italic font-medium tracking-tight leading-none">Colori accento e interfaccia</p>
                  </div>
                  <div className="flex flex-wrap items-center gap-8">
                    <div className="flex gap-2.5 bg-slate-50 dark:bg-slate-800 p-2.5 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-inner">
                      {Object.entries(ACCENT_COLORS).map(([key, { hex }]) => (
                        <button key={key} onClick={() => { setAccentColor(key); localStorage.setItem('accentColor', key); }} className={`w-9 h-9 rounded-full border-4 transition-all ${accentColor === key ? 'border-slate-900 dark:border-white scale-125 shadow-lg' : 'border-transparent opacity-60'}`} style={{ backgroundColor: hex }} />
                      ))}
                    </div>
                    <button onClick={() => { const next = theme === 'light' ? 'dark' : 'light'; setTheme(next); }} className="flex items-center gap-3 px-8 py-4 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-2xl font-black text-[10px] uppercase shadow-xl italic leading-none transition-all active:scale-95">{theme === 'light' ? <><Moon size={18}/> Dark Mode</> : <><Sun size={18}/> Light Mode</>}</button>
                  </div>
               </div>
             ...
             <tbody>
                {currentMonthLogs.map(log => (
                   <tr key={log.id} style={{ borderBottom: '1px solid #eee' }}>
                      ...
                      <td style={{ padding: '18px 15px', fontSize: '12px', fontWeight: 900, textTransform: 'uppercase', color: log.type === 'work' ? '#000' : '#888' }}>{(log.type || 'N/D').replace('_', ' ')}</td>
      ...
      {logToDelete && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[100] flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 p-12 rounded-[3.5rem] shadow-2xl max-w-sm w-full border-4 border-slate-50 dark:border-slate-800 text-center animate-in zoom-in-95 duration-200">
            <div className="w-20 h-20 bg-red-100 rounded-full flex items-center justify-center mx-auto mb-6 text-red-500 animate-bounce"><Trash2 size={40} /></div>
            ...
            <p className="text-xs text-slate-400 font-bold mb-10 uppercase tracking-widest leading-none">L'operazione è definitiva nel Vault.</p>
            <div className="grid grid-cols-2 gap-5">
              <button onClick={() => setLogToDelete(null)} className="p-5 bg-slate-100 dark:bg-slate-800 rounded-2xl font-black text-[10px] uppercase text-slate-500">No</button>
              <button onClick={async () => { await deleteDoc(doc(db, 'artifacts', APP_ID, 'users', user.uid, 'work_logs', logToDelete)); setLogToDelete(null); }} className="p-5 bg-red-600 text-white rounded-2xl font-black text-[10px] uppercase shadow-xl shadow-red-500/20 active:scale-95 transition-all">Sì, Elimina</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showDownloadConfirm && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[100] flex items-center justify-center p-4">
            <div className="bg-white dark:bg-slate-900 p-12 rounded-[4rem] shadow-2xl max-w-sm w-full text-center border-4 border-slate-50 dark:border-slate-800 animate-in zoom-in-95 duration-200">
              <div className={`w-24 h-24 bg-${accentColor}-100 rounded-[2.5rem] flex items-center justify-center mx-auto mb-8 text-${accentColor}-600 shadow-xl`}><Download size={40} className="animate-bounce" /></div>
              ...
              <p className="text-xs text-slate-500 mb-12 font-medium italic leading-relaxed uppercase tracking-widest">Il Vault elaborerà un PDF professionale ottimizzato per la visualizzazione mobile.</p>
              <div className="grid grid-cols-2 gap-5">
                <button onClick={() => setShowDownloadConfirm(false)} className="p-6 bg-slate-100 dark:bg-slate-800 rounded-3xl font-black text-[10px] uppercase text-slate-400 tracking-widest active:scale-95 leading-none">Annulla</button>
                <button onClick={confirmDownload} disabled={isGeneratingPDF} className={`p-6 text-white rounded-3xl font-black text-[10px] uppercase shadow-2xl bg-${accentColor}-600 flex items-center justify-center gap-3 active:scale-95 transition-all italic leading-none`}>
      ...
      {showDeleteRecoveryModal && (
          <div className="fixed inset-0 bg-black/85 backdrop-blur-2xl z-[120] flex items-center justify-center p-4 text-center">
             <div className="bg-white dark:bg-slate-900 p-12 md:p-14 rounded-[4rem] w-full max-w-sm border-4 border-red-500 relative animate-in zoom-in-95 shadow-2xl shadow-red-500/20 mx-auto">
                <button onClick={() => setShowDeleteRecoveryModal(false)} className="absolute top-10 right-10 p-2.5 bg-slate-100 dark:bg-slate-800 rounded-full text-slate-400 hover:text-red-500 active:scale-90"><X size={24}/></button>
                <div className="w-24 h-24 bg-red-100 rounded-full flex items-center justify-center mx-auto mb-8 animate-pulse text-red-600"><Key size={48}/></div>
                ...
                <p className="text-[10px] text-slate-500 font-black mt-2 italic mb-10 uppercase tracking-[0.2em] leading-relaxed">Inserisci la tua Chiave Privata di 16 cifre per autorizzare la distruzione dei dati.</p>
                <form onSubmit={async (e) => {
                    ...
                    if (securityDoc.exists() && securityDoc.data().recoveryCode === deleteRecoveryInput) {
                       setShowDeleteRecoveryModal(false); setShowDeleteFinalConfirm(true);
                    } else alert("Chiave Errata.");
                }} className="space-y-8">
                   <input type="text" placeholder="XXXX-XXXX-XXXX-XXXX" className="w-full p-6 bg-slate-50 dark:bg-slate-800 dark:text-white border-2 border-slate-100 rounded-3xl font-mono text-center font-black outline-none uppercase tracking-[0.2em] shadow-inner" value={deleteRecoveryInput} onChange={e => setDeleteRecoveryInput(e.target.value.toUpperCase())} maxLength={19} />
                   <button type="submit" className="w-full bg-red-600 text-white p-6 rounded-[2rem] font-black uppercase shadow-2xl active:scale-95 transition-all tracking-[0.3em] text-xs italic">Autorizza Wiping</button>
                </form>
             </div>
          </div>
      )}
      ...
      {showDeleteFinalConfirm && (
          <div className="fixed inset-0 bg-black z-[200] flex items-center justify-center p-4 animate-in fade-in duration-1000">
             <div className="text-center text-white animate-in zoom-in-95 duration-500 px-6">
                ...
                <h2 className="text-5xl font-black uppercase italic mb-8 tracking-tighter leading-none">ADDIO VAULT?</h2>
                <p className="text-slate-500 mb-14 max-w-sm mx-auto italic font-black leading-relaxed uppercase tracking-[0.2em] text-[10px]">Il tuo spazio privato e i log verranno vaporizzati permanentemente.</p>
                <div className="space-y-8 flex flex-col items-center">
                   <button onClick={handleDeleteAccount} className="w-full max-w-xs bg-red-600 p-7 rounded-3xl font-black uppercase tracking-[0.4em] shadow-2xl hover:bg-red-700 active:scale-95 transition-all text-sm italic">Sì, Distruggi Tutto</button>
                   <button onClick={() => setShowDeleteFinalConfirm(false)} className="text-slate-600 font-black uppercase tracking-widest hover:text-white transition-all text-[10px] italic">Annulla</button>
                </div>
             </div>
          </div>
      )}
      ...
      {showGuideModal && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[150] flex items-center justify-center p-4 animate-in fade-in duration-500">
             <div className="bg-white dark:bg-slate-900 p-10 md:p-14 rounded-[4rem] w-full max-w-md shadow-2xl border border-slate-200 dark:border-slate-800 text-left overflow-y-auto max-h-[90vh]">
                ...
                <div className="text-center mb-10">
                   ...
                   <p className="text-[10px] text-slate-400 font-black uppercase tracking-[0.4em] italic leading-none">Guida Installazione</p>
                </div>
                <div className="space-y-8">
                  <div className="bg-slate-50 dark:bg-slate-800/50 p-6 rounded-[2rem] border border-slate-100 dark:border-slate-800">
                    <h3 className="font-black text-slate-900 dark:text-white flex items-center gap-4 mb-4 uppercase italic text-xs tracking-widest leading-none"> iOS (iPhone)</h3>
                  ...
                  <div className="bg-slate-50 dark:bg-slate-800/50 p-6 rounded-[2rem] border border-slate-100 dark:border-slate-800">
                    <h3 className="font-black text-slate-900 dark:text-white flex items-center gap-4 mb-4 uppercase italic text-xs tracking-widest leading-none">🤖 Android</h3>
                    <ol className="text-[11px] text-slate-600 dark:text-slate-300 space-y-3 list-decimal list-inside font-bold italic leading-relaxed uppercase tracking-tight">
                ...
                <button onClick={() => setShowGuideModal(false)} className={`mt-10 w-full bg-slate-950 dark:bg-${accentColor}-600 text-white p-5 rounded-2xl font-black uppercase shadow-xl italic tracking-widest`}>Capito</button>
             </div>
          </div>
        )}
    </div>
